...
class Database():
    def __init__(self, motions, feature_mapping, selected_feature_names, selected_feature_weights) -> None:
        ...
        self.joint_name, self.joint_parent, self.joint_offset = motions[0].joint_name, motions[0].joint_parent, motions[
            0].joint_offset
        ...
        for bvh_item in motions:
            ...
            tmp_pos, tmp_rot = forward_kinematics_with_channel(self.joint_parent, self.joint_channel, self.joint_offset,
                                                               true_motion_data)
            ...
            filtered_fwd_direction = filtered_fwd_direction / np.linalg.norm(filtered_fwd_direction, axis=1,
                                                                             keepdims=True)
            ...
            true_motion_data[:, 9:12] = (filtered_sim_rotation.inv() * R.from_euler('XYZ', true_motion_data[:, 9:12],
                                                                                    degrees=True)).as_euler('XYZ',
                                                                                                            degrees=True)

            self.motion_data.append(true_motion_data)

            # Extract the data terms 
            pos, rot = forward_kinematics_with_channel(self.joint_parent, self.joint_channel, self.joint_offset,
                                                       true_motion_data)
            ...
            local_rot = R.from_euler('XYZ', true_motion_data.reshape(-1, 3), degrees=True).as_quat().reshape(
                true_motion_data.shape[0], -1, 4)
...
class CharacterController:
    ...
    def update_state(self, desired_pos_list, desired_rot_list, desired_vel_list, desired_avel_list, current_gait):
        ...
        if np.linalg.norm(self.desired_vel_change) < eps and np.linalg.norm(
                self.controller.desired_velocity_change) > eps:
            should_search = True

        if np.linalg.norm(self.desired_rot_change) < eps and np.linalg.norm(
                self.controller.desired_rotation_change) > eps:
            should_search = True
        ...
        if should_search:
            ...
            if 'trajectoryRot2D' in self.db.feature_names:
                ...
                future_rot = (self.global_bone_rot.inv() * R.from_quat(future_rot[:, [1, 2, 3, 0]])).apply(
                    np.array([0, 0, 1]))
        ...
        joint_translation, joint_orientation = forward_kinematics_with_channel(self.db.joint_parent,
